        ...
        case 'NAVIGATE_DEPRECATED':
          {
            ...
            const path = action.type === 'NAVIGATE' && action.payload.path != null ? action.payload.path : key === route.key ? route.path : undefined;
            const nextRoute = key !== route.key ? {
              key,
              name: route.name,
              path,
              params
        ...
        case 'REPLACE_PARAMS':
          {
            ...
            if (nextState !== null) {
              ...
              if (index != null) {
                const route = action.source ? nextState.routes.find(route => route.key === action.source) : nextState.routes[index];
                if (route === undefined) {
                  return nextState;
                }
                const historyItemIndex = state.history.findLastIndex(item => item.type === 'route' && item.key === route.key);
        ...
        case 'PRELOAD':
          {
            ...
            } else if (backBehavior === 'fullHistory' && newRoute.params !== route.params) {
              const historyItemIndex = history.findLastIndex(item => item.type === 'route' && item.key === route.key);
              if (historyItemIndex !== -1) {
                history = [...history];
